...
async def create_checkout(input: CheckoutRequest, user: dict = Depends(get_authenticated_user)):
    ...
    # webhook_url = f"{input.origin_url}/api/webhook/stripe"
    # stripe_checkout = StripeCheckout(api_key=stripe_api_key, webhook_url=webhook_url)
    
    success_url = f"{input.origin_url}/order-success?session_id={{CHECKOUT_SESSION_ID}}"
    cancel_url = f"{input.origin_url}/cart"
    ...
    try:
        ...
        if webhook_response.event_type == "checkout.session.completed":
            await db.payment_transactions.update_one(
                ...
                {"$set": {"payment_status": webhook_response.payment_status, "status": "completed"}}
            )
